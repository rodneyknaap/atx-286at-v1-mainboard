...
The REV3D system is now fully functional and able to boot and run software. I am still sorting out the optimal System controller design for a few typical clock frequencies of the 286 CPU. The REV3D design has certain advantages and improvements because all the manual development revisions which I have wired in on the REV1 system are now an integral part of the design. Besides this, the bus control is now done by CPLD chips as well and we don't need a lot of bus transceivers and address/data bus latches anymore to create a system.
Besides this there is also a new EMS system integrated into the design which eventually is meant to support the RealDOOM project by sqpat here on GitHub (https://github.com/sqpat/RealDOOM) of which I am a huge fan. Basically running RealDOOM is a great application to demonstrate the coolest DOOM gameplay fun which the game is able to offer including music and sound effects, tweakable graphics detail and other cool stuff, and also giving a really great demonstration of what the 286 CPU is capable of doing in an optimal system.  

So a few points I want to mention about the REV3D:
...
- SMD chip soldering is less difficult than it may seem at first, the result is based on using the right method for soldering. It's a matter of using enough good no-clean flux, pre-loading the pads with a coat of leaded solder using flux, lots of solder applied on a large soldering tip and just wipe with fast movements along the pad directions without pressing on the pads in the movement. Then positioning the chip very carefully, wiping across a few legs of the IC on all edges to fix the chip in position, then apply more flux and wipe each leg of the chip individually to let the solder from the pad underneath flow onto the pin. This process can be done with a larger wedge shaped iron tip to make sure enough heat is transfered onto the chip leg to let the solder joint flow together. After soldering it's a matter of gently feeling all the legs if they are solid, and retouching any that are not. A final few checks should make sure that the SMD chips are completely solid to the board. At the moment I am using some cheap microscope glasses with a strong LED lighting for this work so I can keep a better view of the work.

Kind regards,

Rodney

Updated last on october 25th, 2025.
