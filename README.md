...
## Cooperation
This project and developing such a new PC system design is a huge work. No one can truly appreciate or understand how much work this project constituted unless they have done a similar effort and completed a similar sized work, which would interest me a lot to exchange experiences in such a case. I will consider if cooperation could be possible on a per-case basis, depending on if the cooperation feels good and positive, and depending on what area of cooperation I would be in need of. If you want to know in what areas the project development can use help, you can read my VCF thread where I have written from time to time about potential areas where I could use the help. Of course, in case the problem or issue etc has since then already been developed or solved, it will no longer be needed to help in that regard. I appreciate and thank everyone for all intentions and offers for help, in the spirit of friendship and mutual respect. If it intuitively feels good to me, and would benefit an area of the project where I feel I could use some help, in that case I would be happy to take you up on the offer.

## Conclusion of this first design stage
Publishing the updated final schematic and resulting gerber files will be the last work done in order to finalize this CPLD iteration of my development. After that I will focus exclusively on the next project.
...
## Final revision of CPLD 286 system  
After considering various aspects of designs which follow logically from this initial CPLD project, I decided that the CPLD edition of my 286 system deserves a final update before definitely moving on to FPGA technology. The reasons for this decision are that departing from CPLD also poses a range of new developmental problems in the design, while finishing the project in a higher level integration CPLD version still uses native 5V devices, while at the same time attempting to eliminate a tri state TTL device based system bus structure. There are also various design difficulties associated with the CPLD devices of which the biggest limitation is a maximum of six different OE functions per CPLD package. This forced the design into a certain direction which I did not at first wanted to accept, however I managed to find a satisfactory compromise after doing 4 different almost complete mainboard designs where the logic is all located inside CPLDs which are pieced together to compose a PC/AT system. After completing the design in KiCad and 5 different quartus projects that compose the system, I had a closer look at VLSI based solutions of the time period, which yielded some similarities but also some differences with my system design. I also took some leaps in terms of assembling an address generator CPLD where the address bus is completely merged inside a single CPLD package. Only the generation of the buffered X-address bus is done in another package due to the mentioned OE function limit, however this aspect of the X-Address bus is only a simple buffering function for X-devices running on this bus while being addressed by the CPU. The new design is located in my REV3 repository here in my GitHub projects: https://github.com/rodneyknaap/ATX-286AT-V3-mainboard

If you are interested in this REV1 design, possibly you may like to take a look at the REV3 project. Current status of this project is that the mainboard design and the initial versions of the 5-CPLD quartus projects are finished, there is also a SRAM module design which is connected directly to the mainboard instead of on the ISA slot connector, and I am working on an EMS concept so we can swap out EMS pages in the entire first 1MB region. This EMS capable memory configuration is my attempt to support the RealDOOM project by sqpat of which I am a huge fan. I feel that his project is one of the best applications to run on a faster 286 CPU. If this is successful, it will probably be supported by a custom driver implementation in the future. The RealDOOM project by sqpat can be found here: https://github.com/sqpat/RealDOOM
He is working on supporting the DOOM music and sound effects, and transitioning RealDOOM into a full assembly based project. He plans to implement optional rendering modes so you can observe the game mechanics in more detail by using flat style game graphics elements. Make sure to check out his demonstration videos which are really cool to watch!

The REV3 design has implemented several bus adaptations which will need to be tested and verified, where I took some larger steps in order to be able to use CPLDs. I also will experiment with using an 8 bit mode 1 megabit ROM and later will experiment with direct attachment of SRAM memory to the 286 databus. This design structure is also seen in the faster VLSI PC/AT systems such as the second generation 5 device chipset mainboards and for example the TOPCAT chipset by VLSI. Directly attaching the SRAM to the CPU data bus may be another step in the future to advance to much faster memory speeds in a PC/AT system design.
Another goal of the REV3 system design is doing further attempts to achieve zero wait state efficiency from the 286 CPU. To see the entire scope of the design, I invite you to look at my REV3 CPLD project. After finishing the work on development and testing of this REV3 CPLD PC/AT design based on the IBM 5170, I will finally move on to FPGA in the next PC/AT design as mentioned before, which I will do in the combined 286/486 FPGA project for which I have also started a separate repository. I am working hard to transition and evolve the PC/AT design into openly published projects so we can preserve the amazing original designs by IBM's PC team led by Don Estridge! The first part of this work was reverse engineering the PAL logic and implementing the design into CPLD components for system control and decoding operations. The next phase will be the REV3 CPLD design, and after that I will work on an even higher integration FPGA iteration. Anyone who wants to help with the FPGA stage is welcome to contact me. For example, I am looking for help to integrate the core AT controller chips such as 8237, 8042, 8259 and 8254 into FPGA code. If you are experienced in Verilog or VHDL and feel you may be able to help, please let me know.

Kind regards,

Rodney

Updated last on july 31st, 2025.
