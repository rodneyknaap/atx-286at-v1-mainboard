...
![A 3D view picture of the mainboard design generated by KiCad](REV2C_INPROGRESS_01.png)  

# A possible TTL/PLCC REV2C design may be developed!  
I am currently (sep 2026) looking into creating a new design based on this REV1 project, where I am summarizing all the developments manually done on this project with a few enhancements from the work on the REV3E system. I am working hard on the new design to combine the best developments into a TTL / PLCC CPLD system similar in look and feel to the REV1, however with much improved technology and design structure. Theoretically this system may be able to operate at high clock speeds above 20MHz, and the idea is to also offer the EMS system as used with the RealDOOM driver developed in cooperation with developer sqpat here on GitHub.  

The new design is going to be named REV2C and I will attempt to design the entire system onto a single mainboard PCB, including the system RAM soldered directly onto the mainboard. I have designed the 84 pin synchronous System controller and Address bus driver CPLDs, and I will work on the 84 pin IO decoder. The design will feature EMS bus control using TTL chips, on par with the existing REV1 design style. XMS and EMS here will also be flexible in assignment, which means that populated EMS SRAMs will be able to double as XMS memory, so all of the SRAMs can be used as XMS in the system, and when the EMS driver is loaded, it activates the EMS system which takes 4 designated chips of the SRAM memory for the EMS memory pool. So the idea is to populate at least 2MB (low and high byte 1MB) for the base memory, and add at least 4MB of EMS. So in this situation you get 6MB of total RAM, of which the top 4MB can be assigned as EMS by loading the driver.

Otherwise I have revised the bus structure in order to accommodate the limited CPLD pin numbers and maximize their use. I will attempt to drive the DMA mid address bits on the CPU side which, just like with the 74LS612 DMA page register, during DMAC active periods will not pose any problem because the 80286 CPU will be in HOLD condition and tri states the address lines on the CPU side. So the Address bus driver can take a single set of address inputs and generate the SRAM chip selects and SA bus lines.  

I have removed the LAxx transceiver in the REV2C so we can drive these lines with the 80286 directly. This also benefits VGA controller decoding speeds and hopefully we can control the VGA cycles at full processing speeds using a fast Cirrus Logic VGA controller, and if possible at 20MHz and above. When the 80286 and DMACs are both inactive, a possible ISA bus master can take the bus and the Address bus driver will provide the necessary memory decoding during the bus master periods.  

Otherwise I will do a lot of cleanup and optimizing the space much further. The image provided is merely an early rough draft to illustrate the idea. More bus chips will be added to complete the system and I will first create additional space in the dense CPU and system RAM areas. The X-bus devices will be mostly moved down and away from the dense CPU areas. More detailed images will follow as the work progresses. Because of using TTL ICs, this will require more space for the whole design. So this largely depends on further optimizing all the design areas and freeing up the maximum possible board area for the CPLDs, the 80286, the EMS system and the SRAMs.  

Please note: if you would like to see this REV2C system developed and built, and you would be willing to provide some support to the REV2C build, please consider contributing in the costs for parts and the PCB order. You can reach out to me by email, see my profile or you can use the message form on www.knaapic.nl where the website can also send me a message. If I see substantial interest and help, this may motivate me and improves the chances for me to be able to cover the costs to bring out a fully verified design. Currently I am considering that possibly a TTL based design may be able to operate at high speeds with the help of faster system SRAM chips, for example 45ns is known in the REV3E system to support at least 22.4MHz clock speeds. It is my hope that we may be able to match this speed with this design type as seen in REV1.  
...
# Synchronous REV3E System control logic 00395 now also confirmed in REV2C precursor system   
A while ago I have done a lot of work on the REV1 System controller where I completely moved SA0 and SBHE output logic into the system controller and eliminated U89. However while working on even more synchronous System control in the REV3E system I had gone even further to reduce the state machine to use only two bits and now even more areas are developed into fully synchronous logic, including the two bit state machine. Also I had reduced the gate logic on the cycle switcher, improved DEN and DT_R even further, eliminated AIOW from being necessary and simply working with IOW, no longer using any asynchronous CLEAR_CUR_CYC mechanism anywhere. Anyway all these REV3E updates had not been added to the REV2C System controller yet.  

So today I continued a few more things in the REV2C precursor system, among other things I removed the LAxx transceiver and directly ran the slots LAxx lines from the CPU side address lines, however I started to run into some weird timing issues. At first I thought it must be reflections in the ISA lines coming back to the Harris 80286 address lines, so I added a pull up resistor network of 10k on the LAxx lines. This improved things a little but still not entirely perfect. I started to see RAM block errors in the system BIOS selftests and VGA artifacts, and I was not able to run the VGA card RAM access at full speed with a clock of 16MHz. Then I suddenly realized that last time during testing I didn't fully finish the development and testing range of System controller updates. Even though I was working on the address bit configuration, I still started to get a feeling that likely the cycle timing was also involved. So I went over the 00395 System controller in the REV3E and updated all the latest improvements into the REV2C System controller. At the first test this immediately solved all the issues I was seeing while testing initially today.  

I will rerun the previous tests because I still want to retry driving the VGA controller directly from the 286_A20 line instead of the gated A20. Theoretically this should work because the VGA RAM is not included in the BIOS system RAM checks, and since the VGA RAM is only accessed at 0A0000 and 0B0000, there really should be no issue whether the 80286 CPU is in real mode or not. Anyway, testing this is a little tricky using the manually wired REV1-to-REV2C system since there the RAM card is still on the slots. So when LA20 is driven by the CPU directly, of course we will get some gate A20 errors from the BIOS. So for testing this I would need to run the gated A20 through the 10 pin connector to the memory card, while driving LA20 on the slot with the ungated CPU A20. I am simply testing such things because I want to confirm this to work for limited pin count systems other than the REV3E where I may want or rather need to reduce some ICs or pins in the system. So I will retest the previously wired version of today and reprogram the memory decoder for another test. Today I retried pre-latching all the LAxx lines and not latching the end CS outputs, and finally I found why this didn't work well previously. Apparently the Cirrus Logic VGA controllers signal MEM_CS_16_n only early on in the cycle, and later on this signal is de-asserted. This leads to lots of VGA RAM cycles failing while interpreting the FSYS_16 signal. Well, the solution was simply latching the MEM_CS_16_n input during the ALE pulse. After doing that the VGA cycles were completely functional again. So in the system design this is an important consideration to always latch the MEM_CS_16_n line. With MEM_CS_16_n this is different from IO_CS_16_n because MEM_CS_16_n on the slots is always derived from the LAxx signals which are unlatched. So this should be observed whether the driving card is using the LAxx timing or latching the MEM_CS_16_n output itself. So it's better to be safe and latch it as well before it goes to the System controller. So that was the final solution to get the pre-latch memory decoder fully functional which I feel is a better approach. For the slow cycle decoders in the memory decoder circuits we can use the CPU lines directly for a faster decode, where we will be latching the current slow cycle status anyway so we only need the signal at a designated point at the cycle start.   
